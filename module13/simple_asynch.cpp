...
int main(int argc, char** argv)
{
    ...
    errNum = clEnqueueNDRangeKernel(queue0, kernel0, 1, NULL, (const size_t*)&gWI,
      (const size_t*)NULL, 0, 0, &event0);

 	cl_event event1;
 	errNum = clEnqueueMarker(queue1, &event1);

    errNum = clEnqueueNDRangeKernel(queue1, kernel1, 1, NULL, (const size_t*)&gWI,
      (const size_t*)NULL, 0, 0, &event0);
 	...
 	// Read back computed data
   	clEnqueueReadBuffer(queue0,buffer0,CL_TRUE,0,
            sizeof(int) * NUM_BUFFER_ELEMENTS * numDevices,
            (void*)inputOutput0,0,NULL,NULL);
   	clEnqueueReadBuffer(queue1,buffer1,CL_TRUE,0,
            sizeof(int) * NUM_BUFFER_ELEMENTS * numDevices,
